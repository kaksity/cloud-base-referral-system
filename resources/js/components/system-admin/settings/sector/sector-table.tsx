import ProcessDeleteSectorController from '@/actions/App/Http/Controllers/Web/SystemAdmin/Settings/Sector/ProcessDeleteSectorController';
...
export default function SectorTable({
    ...
    sectors: Sector[];
    onEditSectorClicked: (sector: Sector) => void;
    paginationPayload: {
        meta: PaginationMeta;
    ...
    function deleteSector(sectorId: string) {
        form.delete(ProcessDeleteSectorController.url({ sectorId }), {
            onSuccess: () => {},
            onError: () => {},
        });
    }

    const columns: ColumnDef<Sector>[] = React.useMemo(
        () => [
            {
                accessorKey: 'name',
                header: () => <span className="text-sm font-semibold">Sector Name</span>,
                cell: ({ row }) => {
                    ...
                    return (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => onEditSectorClicked(sector)}>Edit</DropdownMenuItem>
                                <DropdownMenuItem className="text-red-600" onClick={() => deleteSector(sector.id)}>
                                    Delete
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                            ...
                            <TableRow className="hover:bg-transparent data-[state=selected]:bg-transparent">
                                <TableCell colSpan={columns.length} className="h-48">
                                    <EmptyState
                                        ...
                                        description="Your sectors will show up here as soon as a sector is created."
                                    />
                                </TableCell>
                            </TableRow>
                        )}
